  ...
  return (
    <div className={`rounded-2xl ${tv ? "bg-zinc-900 p-6" : "border bg-card p-5"}`}>
      <div className="flex items-baseline justify-between gap-2">
        ...
        {fila.fila.length > 0 && (
          <p
            className={`shrink-0 font-cond text-xs ${tv ? "text-zinc-400" : "text-muted-foreground"}`}
          >
      ...
      {emAndamento && (
        <div className={`mt-3 rounded-xl p-4 ${tv ? "bg-emerald-900/60" : "bg-success/10"}`}>
          <p
            className={`text-xs font-semibold uppercase tracking-wide ${tv ? "text-emerald-300" : "text-success"}`}
          >
          ...
          <div className={`mt-2 space-y-1 ${tv ? "text-xl text-white" : "text-sm"}`}>
            <p className="flex items-center justify-between gap-3">
              <span className="truncate font-medium">
                {nome(emAndamento.luta.atleta1InscricaoId)}
              </span>
              <span className="font-black tabular-nums">
                {emAndamento.luta.pontos1}
              </span>
            </p>
            <p className="flex items-center justify-between gap-3">
              <span className="truncate font-medium">
                {nome(emAndamento.luta.atleta2InscricaoId)}
              </span>
              <span className="font-black tabular-nums">
                {emAndamento.luta.pontos2}
              </span>
            </p>
          </div>
        </div>
      ...
      <ul className={`mt-3 space-y-1.5 ${tv ? "text-lg text-zinc-200" : "text-sm"}`}>
        {fila.fila
          ...
          .map((i) => (
            <li key={i.luta.id} className="flex items-center gap-2">
              <span
                className={`font-cond text-xs ${tv ? "text-zinc-400" : "text-muted-foreground"}`}
              >
                ~{hora(i.horaEstimada)}
              </span>
        ...
        {fila.fila.length === 0 && (
          <li className={tv ? "text-zinc-400" : "text-muted-foreground"}>
            Área concluída ✓
          </li>
        )}
      </ul>
...
/**
 * Grade de áreas do cronograma ao vivo — compartilhada entre a aba Cronograma
 * (dentro do shell do evento) e o modo telão (`/telao`, tela cheia). O
 * `AutoRefresh` mantém os horários estimados atualizados sozinho.
 */
export function CronogramaGrade({
  ...
  return (
    <>
      ...
      <div className={`grid gap-6 ${filas.length > 2 ? "grid-cols-3" : "grid-cols-2"}`}>
        {filas.map((f) => (
