 ...
 * ordem intercalada p/ descanso), intercalação livre entre divisões (corridas
...
describe("montarCronogramaDoEvento — ordem manual", () => {
  it("sem override: intercala p/ dar descanso (definida ⇄ A definir)", async () => {
    await definirOrdem([
      [A.lutas[0].id, null],
      [A.lutas[1].id, null],
      [A.lutas[2].id, null],
    ...
    // A tem 2 lutas definidas de round 1 (a0, a1) + a final indefinida (a2); B
    // tem 1 luta definida (b0). Intercalando, b0 entra ANTES da final de A → o
    // vencedor de a1 descansa em vez de emendar. A quebra em 2 corridas, B no meio.
