import type { Dicionario } from "./pt";

export const en: Dicionario = {
  rodape: "Martial arts competition system",
  nav: {
    eventos: "Events",
    plataforma: "The platform",
    minhasInscricoes: "My registrations",
    minhaArea: "My area",
    painel: "Organizer dashboard",
    ativarOrganizador: "Become an organizer",
    verSite: "View site",
    minhaConta: "My account",
    idioma: "Language",
    sair: "sign out",
    entrar: "Sign in",
  },
  telaoArea: {
    telaCheia: "Fullscreen",
    sairTelaCheia: "Exit fullscreen",
    noTatame: "On the mat",
    proximas: "Up next",
    aguardando: "waiting",
    termina: "ends",
    luta: "match",
    lutas: "matches",
    semLutas: "No matches pending",
    semCronograma:
      "The schedule appears here once the organizer assigns the brackets to the areas.",
    subtituloTelao: "Estimated times · auto-updates",
  },
  evento: {
    todosEventos: "All events",
    inscrever: "Register",
    cronogramaAoVivo: "Live schedule",
    status: {
      aoVivo: "Live now",
      inscricoesAbertas: "Registration open",
      finalizado: "Event finished",
      encerradas: "Registration closed",
    },
    abas: {
      informacoes: "Info",
      checagem: "Check-in",
      chaves: "Brackets",
      cronograma: "Schedule",
      resultados: "Results",
    },
    fichaTecnica: "Event details",
    fatos: {
      modalidade: "Format",
      data: "Date",
      local: "Location",
      ginasio: "Venue",
      circuito: "Circuit",
      faixas: "Belts",
      areas: "Mats",
      inscricoesAte: "Register by",
      pesagem: "Weigh-in",
      chaves: "Brackets",
      pagamento: "Payment",
    },
    modalidades: {
      gi_nogi: "Gi + No-Gi",
      gi: "Gi",
      nogi: "No-Gi",
    },
    pagamento: { pix: "Pix", cartao: "Card" },
    sobre: "About the event",
    sobreFallback:
      "Martial arts championship with registration, bracketing and digital scoreboard by LeagueMat.",
    sidebar: {
      inscricao: "Registration",
      porCategoria: "per division",
      inscricoesFecham: "Registration closes",
      segundaCategoria: "Second division",
      pagamento: "Payment",
      equipes: "Teams",
      confirmada: "confirmed",
      confirmadas: "confirmed",
      sejaPrimeiro: "Be the first to register!",
    },
    faixaNomes: {
      branca: "White",
      cinza: "Grey",
      amarela: "Yellow",
      laranja: "Orange",
      verde: "Green",
      azul: "Blue",
      roxa: "Purple",
      marrom: "Brown",
      preta: "Black",
      vermelha_preta: "Red and Black",
      vermelha_branca: "Red and White",
      vermelha: "Red",
    },
  },
  catalogo: {
    criarEvento: "Create event",
    circuito: "Circuit",
    buscar: "Search event or city",
    filtros: {
      todos: "All",
      abertas: "Registration open",
      gi: "Gi",
      nogi: "No-Gi",
      emBreve: "Coming soon",
    },
    evento: "event",
    eventos: "events",
    destaque: "Featured",
    aoVivoBadge: "Live",
    verEvento: "View event",
    atletaConfirmado: "athlete confirmed",
    atletasConfirmados: "athletes confirmed",
    inscrito: "registered",
    inscritos: "registered",
    sejaPrimeiro: "Be the first",
    inscrever: "Register",
    nenhumEvento: "No events",
    nenhumCorresponde:
      "No events match these filters. Try another search.",
    localADefinir: "Venue TBD",
    status: {
      aoVivo: "Live now",
      inscricoesAbertas: "Registration open",
      emBreve: "Coming soon",
      finalizado: "Finished",
      encerradas: "Registration closed",
    },
  },
  home: {
    slogan: "The mat",
    sloganAccent: "starts here",
    abaixo:
      "Every event available on the platform is right below. Filter by style or status and sign up in two clicks.",
    roleCue: "See the calendar",
    ctaEyebrow: "Running events?",
    ctaTitulo: "Run your",
    ctaAccent: "championship here",
    ctaDesc:
      "Registrations, brackets, mat areas, check-in, scoreboard and results in one place — from the first sign-up to the podium.",
    ctaBtn1: "Explore the platform",
    ctaBtn2: "Create event",
  },
  criarSenha: {
    eyebrow: "Almost there",
    titulo: "Create your password",
    desc: "Your entry is already saved. Pick a password to get back to it whenever you want — the rest of your details are stored.",
    suaConta: "Your account",
    senha: "Password",
    confirmar: "Confirm password",
    botao: "Create password and continue →",
  },
  blocosHorario: {
    luta: "match",
    lutas: "matches",
    abaBlocos: "By division",
    abaLutas: "Match order",
    titulo: "Schedule by division",
    desc: "Call-up goes by age, sex and belt — weight classes run inside the block. Per-match times are not published because they never hold up at a real event.",
    tatame: "Mat",
    tatames: "Mats",
    vazio: "Times show up once the organiser sets the mats.",
    verBlocos: "See schedule by division",
    verLutas: "See the match order",
  },
  inscricao: {
    voltarAoEvento: "Back to event",
    encerradasTag: "Registration closed",
    encerradasMsg: "Registration for this event is not open right now.",
    passos: { dados: "Details", pagamento: "Payment", confirmacao: "Confirmation" },
    passo1: "Step 1 · Your details",
    titulo: "Registration",
    subtitulo:
      "We only show divisions that match your profile. You can pay now or leave it for later — the deadline is the end of registration.",
    erroGenerico: "Error processing registration",
    nomeCompleto: "Full name",
    seuNome: "Your name",
    email: "E-mail",
    nascimento: "Date of birth",
    sexo: "Sex",
    selecione: "Select",
    masculino: "Male",
    feminino: "Female",
    faixa: "Belt",
    pais: "Country",
    academiaEquipe: "Academy / team",
    cpf: "CPF",
    cpfInvalido: "Invalid CPF",
    cpfVerificando: "Checking…",
    cpfContaTitulo: "You already have an account",
    cpfContaDesc: "This CPF is already registered. Sign in to enter — your details come pre-filled.",
    cpfContaBotao: "Sign in",
    cpfContaFechar: "Use another CPF",
    cepBuscando: "Looking up address…",
    cepNaoEncontrado: "Postal code not found",
    cepErro: "Could not look up the postal code",
    cep: "Postal code",
    logradouro: "Address",
    numero: "Number",
    complemento: "Complement",
    bairro: "Neighborhood",
    cidade: "City",
    uf: "State",
    secaoPessoais: "Personal details",
    secaoAtleta: "Athlete details",
    classeIdade: "Age division",
    categoriaPeso: "Weight class",
    absolutoPergunta: "Do you also want the open weight?",
    absolutoNota: "The open weight counts as a second entry",
    sim: "Yes",
    nao: "No",
    categoria: "Division",
    selecionada: "selected",
    semCategoriaCompat: "No division matches your profile in this event.",
    preenchaPre: "Fill in",
    preenchaPos: "to see your divisions.",
    conectorE: "and",
    emSecao: "in",
    voltar: "Back",
    pagarAgora: "Register and pay now",
    pagarDepois: "Register and pay later",
    enviando: "Sending…",
    salvando: "Saving…",
    resumo: {
      atleta: "Athlete",
      faixa: "Belt",
      divisao: "Division",
      categoria: "Division",
      absoluto: "Open weight",
      taxaInscricao: "Registration fee",
    },
    segundaCategoria: "Second division",
    viaPix: "Pay via Pix now or later",
    buscarAcademia: "Type to search your academy",
    limparAcademia: "Clear academy",
    nenhumaAcademia: "No academy found. Try another term.",
    selecioneAcademia: "Select an academy from the list.",
  },
  checkout: {
    voltarAoEvento: "Back to event",
    confirmadaEyebrow: "Registration confirmed",
    confirmadaTitulo: "You're in the bracket",
    aprovadoPre: "Payment approved. Your spot in",
    aprovadoMeio: "is confirmed",
    aprovadoPara: "for",
    aprovadoFim: ". Oss! 👊",
    acompanhe:
      "Follow the live schedule on event day — mat calls appear in real time.",
    verCronograma: "View schedule",
    minhasInscricoes: "My registrations",
    passo2: "Step 2 · Payment",
    pagueComPix: "Pay with Pix",
    vagaReservada:
      "Your spot is held while payment is pending. Approved in seconds.",
    inscricao: "Registration",
    cobrancaExpiradaTag: "charge expired",
    aguardandoPagamento: "awaiting payment",
    total: "Total",
    pixCopiaCola: "Pix copy-and-paste",
    cobrancaTeste: "test charge",
    expiraEm: "Expires in",
    aposLiberada: "— after that the spot is released.",
    cobrancaPixExpirada: "Pix charge expired",
    gerarNovoPix: "Generate new Pix",
    prazoEncerrado:
      "The payment deadline for this championship has passed — the spot was released.",
    simularPagamento: "Simulate approved payment (test)",
    editarDados: "Edit details",
    eventoFallback: "Event",
    statusLabel: "Status",
    statusConfirmada: "Confirmed",
    statusAguardando: "Awaiting Pix",
    atleta: "Athlete",
    faixa: "Belt",
    categoria: "Division",
    pagamentoAprovadoViaPix: "Payment approved via Pix",
    pagamentoViaPix: "Payment via Pix",
    copiarCodigo: "Copy code",
    copiado: "Copied ✓",
  },
  atletas: {
    titulo: "Check-in",
    subtitulo: "Registered by division · search by athlete or academy",
    ninguemInscrito:
      "No one registered yet — athletes appear here once registration starts.",
    inscritos: "registered",
    confirmadosSelo: "confirmed",
    pendentesSelo: "pending",
    buscar: "Search athlete or academy…",
    agruparPor: "Group by",
    porDivisao: "Division",
    porAcademia: "Academy",
    imprimir: "Print",
    listaAtletas: "Athlete list",
    colAtleta: "Athlete",
    colCategoria: "Category",
    colPais: "Country",
    colStatus: "Status",
    geradoEm: "Generated on",
    todosPaises: "All countries",
    nenhumEncontrado: "No athlete found",
    confirmado: "confirmed",
    confirmados: "confirmed",
    pendente: "pending",
    pendentes: "pending",
    verChave: "View bracket",
    semAcademia: "No academy",
    statusConfirmado: "Confirmed",
    statusPendente: "Pending",
  },
  minhas: {
    titulo: "My registrations",
    semInscricoesPre: "You don't have any registrations yet —",
    encontreEvento: "find an event",
    semInscricoesPos: "to get started.",
    statusAguardando: "Awaiting payment",
    statusConfirmada: "Confirmed",
    statusCancelada: "Cancelled",
    statusReembolsada: "Refunded",
    checkinInfo: "Check-in: show the QR at weigh-in · code",
    qrAlt: "Check-in QR",
    totalAPagar: "Total due",
    umaInscricao: "1 entry",
    nInscricoes: "entries",
    pagarAgora: "Pay now",
    pagueAte: "pay by",
    prazoEncerrado: "Payment deadline passed",
    nenhumaAinda: "No registrations yet.",
    avisoSing: "registration awaiting payment",
    avisoPlur: "registrations awaiting payment",
  },
  chavesTab: {
    titulo: "Brackets",
    publicada: "published",
    publicadas: "published",
    vazioSemCat: "Brackets show up here once the organizer creates the divisions.",
    vazioSemChave: "No brackets published yet — check back closer to the event.",
    colDivisao: "Division",
    colInscritos: "Registered",
    colTatame: "Mat",
    nota: "Call-up times are per division (age, sex and belt) and live in the schedule. Tap a division to see the full bracket.",
    todasAsChaves: "All brackets",
    podio: "Podium",
    campeao: "Champion",
  },
  lutasTab: {
    vazio: "Matches show up here once the organizer creates brackets and assigns mats.",
    buscar: "Search athlete or academy…",
    todas: "All",
    luta: "match",
    lutas: "matches",
    nenhumaEncontrada: "No match found",
  },
  cronogramaTab: {
    titulo: "Schedule",
    subtitulo: "Schedule by division · match order · search by athlete or gym",
    modoTelao: "Big-screen mode",
  },
  resultadosTab: {
    abaPodios: "Podiums",
    abaQuadro: "Medal table",
    titulo: "Results",
    subtitulo: "Podium for every finished division",
    vazioTitulo: "No division finished yet",
    vazioDesc:
      "Podiums show up here as brackets are closed. Until then, follow along in the brackets and the schedule.",
    colDivisao: "Division",
    concluida: "finished",
    concluidas: "finished",
    ouro: "1st",
    prata: "2nd",
    bronze: "3rd",
    quadroTitulo: "Medal table",
    quadroDesc: "Gyms by medal count, gold first.",
    academia: "Gym",
    semAcademia: "No gym",
    total: "Total",
  },
  placar: {
    catAbrev: "div.",
    semCategorias: "No divisions on this mat",
    grupo: "group",
    grupos: "groups",
    luta: "match",
    lutas: "matches",
    chaveNaoGerada: "Bracket not generated · registered",
    semAtletas: "No confirmed athletes",
    statusEncerrada: "Finished",
    statusEmAndamento: "In progress",
    statusARealizar: "Upcoming",
    fechar: "Close",
    vitoriaPor: "Win by",
    vnt: "ADV",
    pun: "PEN",
    aDefinir: "TBD",
    operarPlacar: "Operate scoreboard",
    dia: "Day",
    aposIntervalo: "After break",
  },
  bracket: {
    rodadaPre: "Round ",
    rodadaPos: "",
    final: "Final",
    semifinal: "Semifinal",
    quartas: "Quarterfinal",
    oitavas: "Round of 16",
    campeao: "Champion",
    jogo: "Game",
    dispensado: "not needed",
    chaveVencedores: "Winners bracket",
    repescagem: "Repechage",
    grandeFinal: "Grand final",
    colocacaoFinal: "Final placement",
    grupo: "Group",
    playoff: "Playoff",
    vitoriasAbrev: "W",
    aguardando: "waiting",
    bye: "bye",
    metodos: {
      pontos: "Points",
      vantagens: "Advantages",
      finalizacao: "Submission",
      decisao: "Decision",
      wo: "W.O.",
      dq: "Disqualification",
    },
  },
  classesIdade: {
    pre_mirim: "Pre-Kids",
    mirim: "Kids 1",
    infantil: "Kids 2",
    infanto_juvenil: "Pre-Teen",
    juvenil: "Teen",
    adulto: "Adult",
    master1: "Master 1",
    master2: "Master 2",
    master3: "Master 3",
    master4: "Master 4",
    master5: "Master 5",
    master6: "Master 6",
    master7: "Master 7",
  },
  admin: {
    organizador: "Organizer",
    sair: "Sign out",
    orgDev: "Dev Organizer",
    status: {
      rascunho: "Draft",
      publicado: "Published",
      inscricoes_encerradas: "Registration closed",
      em_andamento: "In progress",
      finalizado: "Finished",
    },
    dashboard: {
      painel: "Organizer panel",
      meusEventos: "My events",
      novoEvento: "+ New event",
      nenhumEvento: "No events yet",
      criePrimeiro: "Create your first championship and start taking registrations.",
    },
    entrar: {
      // common
      entrar: "Sign in",
      criarConta: "Create account",
      nomeCompleto: "Full name",
      email: "E-mail",
      senha: "Password",
      senhaMin: "(min. 6 characters)",
      criarContaBtn: "Create account →",
      entrarBtn: "Sign in →",
      ou: "or",
      jaTemConta: "Already have an account?",
      aindaNaoTemConta: "Don't have an account yet?",
      criarContaLink: "Create account",
      // neutral panel (login and type choice)
      eyebrowNeutro: "Your account",
      heroTitulo: "All your martial arts",
      heroAccent: "in one place",
      heroDesc:
        "Sign in to register for championships, create events and follow the official ranking.",
      subEntrar: "Sign in to your account to continue.",
      // account-type choice
      escolhaTitulo: "Create account",
      escolhaSub:
        "How will you use LeagueMat? Pick one to start — you can have both later.",
      trocarTipo: "← Choose a different type",
      atletaEyebrow: "Compete",
      contaAtleta: "I'm an athlete",
      atletaCardDesc:
        "Register for championships and follow your brackets, results and record — with a single profile.",
      criarAtletaBtn: "Create athlete account →",
      orgEyebrow: "Organize",
      contaOrganizador: "I'm an organizer",
      orgCardDesc:
        "Create and publish events, generate brackets, run check-in and track registrations and revenue.",
      criarOrgBtn: "Create organizer account →",
      // per-type panel and subtitle (on sign-up)
      atletaArea: "Athlete area",
      atletaHeroTitulo: "Your martial arts",
      atletaHeroAccent: "in one profile",
      atletaHeroDesc:
        "Build your profile once and use it across every stage — registration, brackets, schedule and ranking.",
      atletaSubCriar: "Create your athlete account to register for championships.",
      orgArea: "Organizer area",
      orgHeroTitulo: "Run your championship",
      orgHeroAccent: "end to end",
      orgHeroDesc:
        "Create events, publish registration, generate brackets and follow the official ranking.",
      orgSubCriar: "Create your organizer account to publish events.",
    },
    acesso: {
      ola: "Hi",
      comoEntrar: "How do you want to enter?",
      ambos:
        "Your account has both accesses. Choose where to start — you can switch anytime.",
      escolha:
        "Choose the type of access. You can organize championships or compete as an athlete.",
      competir: "Compete",
      souAtleta: "I'm an athlete",
      atletaDesc:
        "See the championships you've competed in, your results, record and the check-in QR codes for your registrations.",
      entrarComoAtleta: "Enter as athlete →",
      gerir: "Manage",
      souOrganizador: "I'm an organizer",
      orgDesc:
        "Create and publish events, generate brackets, run check-in and track registrations and revenue.",
      entrarComoOrganizador: "Enter as organizer →",
      voltarInicio: "← Back to home",
    },
    ativarOrg: {
      eyebrow: "Athlete account",
      titulo: "Become an organizer",
      desc: "Your account is an athlete account. To create and publish championships, enable organizer access — it's one click, and you keep everything from your athlete account.",
      ativar: "Enable organizer access",
      voltar: "Back",
    },
    campos: {
      imagemCapa: "Cover image (URL)",
      bannerPlaceholder: "https://… (event banner)",
      nomeEvento: "Event name",
      nomePlaceholder: "City Martial Arts Cup 2026",
      inscricoesFechamEm: "Registration closes on",
      inscricoesFecham: "Registration closes",
      dataEvento: "Event date",
      cidade: "City",
      cidadePlaceholder: "São Paulo",
      uf: "State",
      moeda: "Currency",
      endereco: "Address / venue",
      enderecoPlaceholder: "Ibirapuera Gym — Av. Pedro Álvares Cabral, s/n",
      descricao: "Description",
      descricaoPlaceholder: "Formats, rules, schedule, prizes…",
      circuito: "Circuit / season",
      circuitoPlaceholder: "National Circuit 2026",
      modalidade: "Format",
      dataPesagem: "Weigh-in date",
      dataGeracaoChaves: "Bracket generation date",
      dataGeracaoNota: "Planned date to build the brackets — informational only.",
      detalhesCompeticao: "Competition details",
      regulamento: "Rules",
      // event days (one or more time windows per day)
      diasEvento: "Event days",
      diasNota:
        "One or more windows per day — repeat the date to split morning and afternoon. Matches are packed into the windows; the break between them stays free.",
      diaData: "Date",
      horaInicio: "Start",
      horaFim: "End",
      adicionarDia: "+ Add day",
      removerDia: "Remove",
      adicionarTurno: "+ Session (same day)",
      removerTurno: "Remove session",
      turno: "Session",
      intervaloLivre: "Free break",
    },
    moedas: {
      brl: "BRL — Real (Pix)",
      usd: "USD — Dollar",
      eur: "EUR — Euro",
    },
    novo: {
      voltar: "Back",
      deco: "NEW",
      criarEvento: "Create event",
      configureEtapa: "Set up your event",
      subtitulo:
        "Basic event info. Price tiers, divisions, mats and brackets are set up later in the panel.",
      regulamentoNota:
        "Optional sections shown on the event's public page. Each starts empty — use “Insert default text” to start from a template and edit as you like.",
      avisoPre: "Advanced settings —",
      avisoBold: "registration price tiers, divisions, mats and bracketing",
      avisoFim: "— become available in the organizer panel right after you create the event.",
      cancelar: "Cancel",
      criarEventoBtn: "Create event →",
    },
    regCampos: {
      inserirPadrao: "Insert default text",
      limpar: "clear",
      preenchido: "filled",
      placeholder: "Optional — leave blank or insert the default text to edit.",
    },
    comum: {
      cancelar: "Cancel",
      fechar: "Close",
      excluir: "Delete",
      excluindo: "Deleting…",
      salvarAlteracoes: "Save changes",
      salvando: "Saving…",
      abrirMenu: "Open menu",
      fecharMenu: "Close menu",
    },
    nav: {
      eventoAtivo: "Active event",
      novoEvento: "+ New event",
      criarNovoEvento: "+ Create new event",
      todosMeusEventos: "← All my events",
      overview: "Overview",
      inscricoes: "Registrations",
      lotes: "Price tiers",
      lotesInscricao: "Registration tiers",
      categorias: "Divisions",
      chaves: "Brackets",
      areas: "Mats",
      checkin: "Check-in",
      equipe: "Team",
    },
    equipe: {
      titulo: "Event organizing team",
      descricao:
        "Invite people to help organize this event and choose what each one can access. Managing the team and deleting the event stay with you.",
      dono: "Owner",
      voce: "you",
      colaboradores: "Collaborators",
      convidar: "Invite a collaborator",
      convidarDesc:
        "Generate an invite link and send it to the person. They accept, sign in and start helping with the organization.",
      emailOpcional: "Email (optional)",
      gerarConvite: "Generate invite",
      convitesPendentes: "Pending invites",
      pendente: "pending",
      semEmail: "no email",
      copiar: "Copy",
      copiado: "Copied!",
      remover: "Remove",
      papel: "Role",
      papeis: {
        total: "Full access",
        mesario: "Table official",
        recepcao: "Front desk",
      },
      papeisDesc: {
        total: "Everything but the team and deleting the event",
        mesario: "Brackets and mats — whoever runs the table",
        recepcao: "Check-in, weigh-in and registrations",
      },
      personalizado: "Custom",
      podeAcessar: "Can access",
      secoes: {
        evento: "Overview and event details",
        inscricoes: "Registrations",
        lotes: "Price tiers",
        categorias: "Divisions",
        chaves: "Brackets and results",
        areas: "Mats, schedule and scoreboard",
        checkin: "Check-in and weigh-in",
      },
      editarAcesso: "Edit access",
      salvarAcesso: "Save access",
      cancelar: "Cancel",
      escolhaUmaSecao: "Pick at least one section.",
    },
    convite: {
      eyebrow: "Invite",
      titulo: "Help organize",
      descPre: "You've been invited to help organize",
      aceitar: "Accept invite",
      entrar: "Sign in to accept",
      verEvento: "Go to the event",
      jaUsado: "This invite has already been used.",
      invalido: "Invalid or expired invite.",
    },
    editar: {
      verPagina: "View page",
      editarEvento: "Edit event",
      eyebrow: "Edit details",
      titulo: "Event information",
    },
    statusInscricao: {
      confirmada: "Confirmed",
      pendente_pagamento: "Pix pending",
      cancelada: "Cancelled",
      reembolsada: "Refunded",
    },
    overview: {
      paginaPublica: "Public page:",
      publicarEvento: "Publish event",
      encerrarInscricoes: "Close registration",
      preparacao: "Setup",
      eventoPublicado: "Event published",
      pendente: "pending",
      aoVivo: "live",
      lotesPreco: "Price tiers",
      lote: "tier",
      lotes: "tiers",
      gradeCategorias: "Division grid",
      areasDefinidas: "Mats defined",
      chavesGeradas: "Brackets generated",
      gradeCbjj: "category grid",
      vigente: "active",
      nenhumVigente: "none active",
      receita: "Revenue",
      liquidoConfirmado: "net confirmed",
      inscricoesRecentes: "Recent registrations",
      emEquipes: "across",
      equipe: "team",
      equipes: "teams",
      confirmada: "confirmed",
      confirmadas: "confirmed",
      pendentes: "pending",
      inativa: "inactive",
      inativas: "inactive",
      nenhumaInscricao:
        "No registrations yet — share the event's public page.",
      semEquipe: "No team",
    },
    excluirEvento: {
      titulo: "Delete event?",
      descNomePre: "The draft",
      descNomePos: "will be deleted forever. This can't be undone.",
      descSemNome: "This draft will be deleted forever. This can't be undone.",
      confirmar: "Delete permanently",
      rotulo: "Delete event",
    },
    inscricoes: {
    inscricaoLabel: "The entry",
    verDetalhes: "View entrant details",
    contato: "Contact",
    endereco: "Address",
    telefone: "Phone",
    statusLabel: "Status",
    valor: "Amount",
    inscritoEm: "Registered on",
      inscricaoSing: "registration",
      inscricaoPlur: "registrations",
      noTotal: "total",
      abaixoMinimo: "Divisions below the minimum entries",
      abaixoMinimoDesc:
        "Merge into another division (moves the athletes and closes the source) or refund the entrants.",
      de: "of",
      min: "min.",
      fundirEm: "Merge into…",
      fundir: "Merge",
      moverPara: "Move to…",
      cancelarAcao: "cancel",
      reembolsar: "refund",
      nenhumaAinda: "No registrations yet.",
      inscricaoManual: "Manual registration",
      manualDesc:
        "For an athlete who paid another way (cash, waiver) — enters directly as confirmed and is logged in the audit trail.",
      inscreverManualmente: "Register manually",
      adicionar: "Add",
      buscarPlaceholder: "Search by name, academy or division…",
      filtroTodas: "All",
      filtroConfirmadas: "Confirmed",
      filtroPendentes: "Pending",
      filtroInativas: "Inactive",
      nenhumResultado: "No registrations found.",
      erroManual: "Could not register. Check the details and try again.",
    },
    categorias: {
      gradeGerada: "Generated grid",
      categoria: "division",
      categorias: "divisions",
      anos: "years",
      montePre: "Build the grid above and click",
      semGrupo: "No group",
      grupoPreco: "Price group",
      excluirCat: {
        titulo: "Delete division?",
        descPre: "The division",
        descPos: "will be removed from the event.",
        confirmar: "Delete division",
      },
    },
    gerador: {
      presets: {
        adultoCompleto: "Full adult",
        kids: "Kids",
        master: "Master",
        soPreta: "Black belt only",
      },
      presetsLabel: "Presets:",
      tabelaPeso: "Weight table:",
      comKimono: "With gi (Gi)",
      semKimono: "Without gi (No-Gi)",
      giNogiNota: "Gi + No-Gi event: generate one table, switch and generate the other.",
      limparTudo: "Clear all",
      classes: "Classes",
      selecionada: "selected",
      selecionadas: "selected",
      tudo: "All",
      nenhuma: "None",
      sexo: "Sex",
      sexos: "Sexes",
      incluirAbsoluto: "Include absolute",
      faixas: "Belts",
      pesos: "Weights",
      previa: "Grid preview",
      totalCategorias: "Total divisions",
      prontasGerar: "ready to generate",
      gerarPre: "Generate",
      selecioneGrade: "Select the grid",
      somaGrade: "Adds to the current grid, no duplicates · editable later",
      jaNaGrade: "already in the grid",
      tudoNaGrade: "All of this is already in the grid",
    },
    lotes: {
      statusVigente: "Active",
      statusFuturo: "Coming soon",
      statusEncerrado: "Ended",
      conflitoMsg:
        "with overlapping dates — on those days the active price is ambiguous. Adjust the periods (delete and recreate) so each tier covers distinct days.",
      janelasPreco: "price windows",
      faixaPreco: "Price range",
      semLotes: "no tiers",
      precoUnico: "single price",
      ate: "up to",
      vigenteAgora: "Active now",
      nenhumAtivo: "none active",
      linhaTempoPrecos: "Price timeline",
      hoje: "Today",
      nenhumPosicionar: "No tiers to place yet.",
      nenhumCadastrado: "No tiers yet. Create the first one in the form above.",
      sobreposto: "Overlapping",
      dia: "day",
      dias: "days",
      datasSobrepoe: "Dates overlap:",
      segundaInsc: "2nd reg.",
      porInscricao: "per registration",
      excluirLote: {
        titulo: "Delete tier?",
        descPre: "The tier",
        descPos: "will be removed. This can't be undone.",
        confirmar: "Delete tier",
      },
    },
    novoLote: {
      titulo: "New tier",
      nome: "Name",
      nomePlaceholder: "1st tier / Early bird",
      preco: "Price",
      precoPlaceholder: "70.00",
      segundaInscricao: "2nd registration",
      opcional: "optional",
      pacotesPreco: "Price packages (optional)",
      addVariacao: "+ variant",
      variacaoHelp:
        "Different prices per group (Kids, Adulto, Feminino…). Then mark each group's divisions on the Divisions tab.",
      grupo: "Group…",
      varPrecoPlaceholder: "100.00",
      removerVariacao: "Remove variant",
      inicio: "Start",
      fim: "End",
      conflitoPre: "This period overlaps tier",
      conflitoPos: "Choose dates outside the other tiers.",
      duracao: "Duration",
      previa: "Preview",
      segundaAbrev: "2nd",
      adicionarLote: "Add tier",
    },
    chaves: {
      status: {
        rascunho: "Draft",
        publicada: "Published",
        em_andamento: "In progress",
        concluida: "Completed",
      },
      semChave: "No bracket",
      intro:
        "Divisions without confirmed registrations are hidden. Generate as draft, review and publish — a bracket can be regenerated until a match has a result.",
      gerar: "Generate",
      chaveSing: "bracket",
      chavePlur: "brackets",
      emLote: "in bulk",
      imprimirChaves: "Print brackets",
      chavesTitulo: "Brackets",
      // bulk generation dialog (format applied to all brackets)
      loteTitulo: "Generate brackets in bulk",
      lotePergunta: "Format for all brackets",
      loteNota: "Divisions with fewer than 3 athletes use single elimination.",
      publicar: "Publish",
      confirmadoSing: "confirmed",
      confirmadoPlur: "confirmed",
      insuficiente: "not enough to generate a bracket",
      formatoSugerido: "suggested format:",
      campeaoWo: "champion by W.O. (1 athlete)",
      // medal handout (finished bracket / ceremony)
      medalhasMarcar: "Mark medals handed out",
      medalhasEntregues: "Medals handed out",
      medalhasPendentes: "Medals pending",
      medalhasDesfazer: "Undo",
      formatos: {
        eliminacao_simples: {
          nome: "Single elimination",
          desc: "Lose and you're out. Each winner advances to the final. Minimal byes and teammates kept apart in the draw.",
          quando: "The most common — fast and direct, for any number of athletes.",
        },
        eliminacao_dupla: {
          nome: "Double elimination",
          desc: "You're only out after two losses. Losing the main bracket drops you to the repechage — you can still reach the final.",
          quando: "Fairer: one slip doesn't eliminate you. Good when there's time for more matches.",
        },
        round_robin: {
          nome: "Round robin",
          desc: "Every athlete faces everyone else. Ranked by wins, breaking ties on head-to-head and then submissions.",
          quando: "Ideal for small groups (2 to 5) — everyone fights several times.",
        },
        tres_repescagem: {
          nome: "3 athletes with comeback",
          desc: "For divisions of exactly 3 athletes: guarantees each one fights at least twice before the decider.",
          quando: "Divisions with 3 entries.",
        },
        multistage: {
          nome: "Groups + playoff",
          desc: "First round robin within each group, then a knockout among the qualifiers.",
          quando: "Large divisions that need a group stage before the finals.",
        },
        votacao_jurados: {
          nome: "Judges' scoring",
          desc: "No head-to-head: judges score each performance and the ranking comes from the total score.",
          quando: "Events judged by score rather than by match.",
        },
        colocacao: {
          nome: "Placement bracket",
          desc: "Everyone gets a final placement (1st, 2nd, 3rd…). Losers keep fighting to settle their position.",
          quando: "When you want to rank every athlete, not just the podium.",
        },
        melhor_de_tres: {
          nome: "Best of 3 (2 athletes)",
          desc: "A direct series between two athletes: first to win two of three matches takes it.",
          quando: "Divisions with only 2 entries, or superfights.",
        },
      } as Record<string, { nome: string; desc: string; quando: string }>,
      seletor: {
        titulo: "Bracket format",
        atletasConfirmados: "confirmed athletes",
        automaticoNome: "Automatic",
        automaticoDesc: "Let the system pick the format by the number of entries.",
        automaticoNesta: "For this division:",
        recomendado: "recommended",
        emBreve: "coming soon",
        exigeExatamente: "requires exactly",
        exigeMin: "min.",
        atletas: "athletes",
        quandoUsar: "When to use:",
        numJurados: "Number of judges",
        avisoRegerarPublicada:
          "This bracket is published. Regenerating replaces the current bracket (the draw changes) and keeps it published. Only possible while no match has a result.",
      },
      regenerar: "Regenerate",
      gerarChave: "Generate bracket",
      abrir: "open",
      nenhumaCategoriaConfirmada:
        "No division with confirmed registrations yet.",
      chaveRascunhoAviso:
        "Bracket in draft — publish it on the brackets list to enable entering results.",
      filtrarTodas: "All",
      filtroClasse: "All age groups",
      filtroFaixa: "All belts",
      filtroSexo: "Both sexes",
      buscarPlaceholder: "Search division or athlete…",
      nenhumaNoFiltro: "No divisions found.",
    },
    checkin: {
      intro:
        "Scan the athlete's QR with the phone camera (opens the weigh-in screen directly) or search by name / code.",
      confirmados: "Confirmed",
      checkinFeito: "Checked in",
      foraDoPeso: "Over weight",
      buscarPlaceholder: "Athlete name or code (e.g. A1B2C3D4)",
      buscar: "Search",
      aguardando: "Awaiting",
      para: "for",
    },
    checkinAtleta: {
      limiteLabel: "Limit:",
      comKimono: "(with gi)",
      semLimite: "No weight limit",
      inscricaoPre: "Registration",
      naoPodeCheckin: "— can't check in.",
      foraDoPesoMaiusc: "OVER WEIGHT",
      checkinOk: "Check-in OK ✓",
      limiteWord: "limit",
      foraResolva:
        "Resolve it in registration management: move to another division or disqualify, per the event rules.",
      desfazerCheckin: "undo check-in (logged in the audit trail)",
      pesoAferido: "Measured weight (kg)",
      registrarPesagem: "Record weigh-in",
    },
    placar: {
      nenhumLutaProntaEm: "No fight ready in",
      filaVazia: "The queue is empty or waiting for winners of previous fights.",
      voltarAsAreas: "Back to mats",
      naFila: "in queue",
      // next-match picker (pull a match to the mat)
      escolherLuta: "Pick match",
      escolherTitulo: "Next on the mat",
      escolherTexto:
        "Pick which match runs now — the mat schedule readjusts itself.",
      escolherBuscar: "Search by athlete, academy or category…",
      escolherVazio: "No match found",
      escolherFilaVazia: "No pending matches on this mat.",
      escolherAtual: "On the mat",
      escolherAguardando: "Waiting for a winner",
      escolherPuxar: "Run this next",
      escolherFechar: "Close",
      escolherSalvando: "Reordering queue…",
      escolherTrocando: "Switching the mat's match…",
      escolherTrocandoNota: "Readjusting the mat schedule and loading the scoreboard.",
    },
    placarTablet: {
      vantagensLabel: "Advantages",
      punicoesLabel: "Penalties",
      trocarLados: "Switch sides",
      vantagemLetra: "A",
      punicaoLetra: "P",
      pausar: "Pause",
      iniciar: "Start",
      zerar: "Reset",
      encerrarLuta: "End fight",
      confirmarResultado: "Confirm result",
      vencedor: "Winner",
      metodo: "Method",
      confirmar: "Confirm",
      confirmando: "Ending…",
      voltar: "back",
      iniciarParaMarcar: "Start the clock to score points",
    },
    areas: {
      nenhumaCategoria: "No divisions loaded",
      gereGradeAntes:
        "Generate the division grid before structuring the mats — the distribution starts from it.",
      irPara: "Go to",
      numeroAreas: "Number of mats",
      categoriasCarregadas: "Divisions loaded",
      em: "in",
      grupo: "group",
      grupos: "groups",
      estruturarAreas: "Structure mats",
      ajudaOrdena:
        "The system orders the grid from the extremes inward (kids and older masters finish early, the middle — Adult / Master 1 — runs last) and spreads the divisions across the mats balancing the load, building the fight schedule with an estimated time per mat.",
      ordemDoDia: "Order of the day",
      extremosMeio: "extremes → middle",
      tatames: "mats",
      naGrade: "in the grid",
      mediaArea: "Avg / mat",
      statGrupos: "Groups",
      classeSexoFaixa: "class · sex · belt",
      expandirTelaCheia: "Expand to full screen",
      prontoEstruturar: "Ready to structure",
      informePre: "Enter the number of mats and click",
      informeDistribuir: "to distribute the",
      informeMontar: "and build the schedule.",
      cronogramaPorArea: "Schedule by mat",
      fecharTelaCheia: "Close full screen",
      // manual match reordering (drag-and-drop)
      reordenarLutas: "Reorder matches",
      reordenarConcluir: "Done",
      reordenarDica:
        "Drag by the handle to reorder each mat's matches — you can interleave divisions. The order only affects the schedule and the display; the bracket doesn't change.",
      reordenarAlca: "Drag to reorder",
      reordenarAviso: "Depends on a match that comes later",
      reordenarVazio: "Generate the brackets to reorder matches.",
      // day configuration on the areas screen itself
      // move matches/categories between mats in the order editor
      moverEntreAreasDica:
        "Drag by the handle to reorder within the mat — or drop the match on another mat's column to move it there.",
      moverSoltarAqui: "Drop here to bring the match",
      moverCategoriaTitulo: "Move division to another mat",
      moverCategoriaBotao: "Move",
      moverCategoriaAbrir: "Move division",
      moverCategoriaEscolha: "Which division changes mat?",
      moverCategoriaPara: "Move to which mat?",
      moverCategoriaVoltar: "back",
      moverCategoriaVazio: "No divisions on this mat.",
      moverCategoriaProcessando: "Moving the division to the other mat…",
      moverCategoriaProcessandoNota: "Recalculating both mats' schedules.",
      diasEvento: "Event days",
      diasNota:
        "Matches are spread within each day's windows. Repeat the date for morning and afternoon — the break between them stays match-free. Adjust before structuring.",
      rascunhoNota:
        "Structuring saves everything you adjusted in the steps at once: window, mat time, day order and the number of mats.",
      rascunhoPendente: "unsaved changes",
      // structuring mode: automatic × by day (manual)
      modoLabel: "Mode",
      modoAutomatico: "Automatic",
      modoPorDia: "By day",
      porDiaTitulo: "Categories by day",
      porDiaTexto:
        "Pick, per day, the classes, sexes and belts that run on it. Each category goes to the first day whose filter includes it; the rest stay without a day.",
      porDiaSemDia: "without a day",
      porDiaTodasAtribuidas: "Every category has a day",
      estruturarPorDia: "Structure by day",
      porDiaSemFiltro:
        "Select at least one class, one sex and one belt on at least one day.",
      // printable schedule by day
      // organizer-editable match-time table (minutes)
      temposTitulo: "Match time",
      temposTexto:
        "Regulation minutes by age class (kids) and by belt (adult+). The schedule adds 2 min of transition between matches and recalculates the times. An empty field falls back to the CBJJ default.",
      temposKids: "Kids · by class",
      temposAdulto: "Adult+ · by belt",
      temposNotaJuvenil:
        "Juvenile, adult and masters follow the belt row. A single category can still get its own time on the Categories screen.",
      temposMin: "min",
      temposPadrao: "default",
      // editable order of the day (drag the age classes)
      ordemClassesTexto:
        "Drag the classes to set the sequence they run in. The default is the extremes-to-middle funnel (kids and older masters finish early, the middle runs last) — change it freely. The order applies next time you structure the mats.",
      ordemClassesPadrao: "Following the default rule (extremes → middle)",
      ordemClassesPropria: "Order set by you",
      ordemClassesRestaurar: "Restore default",
      // areas setup wizard
      assistenteTitulo: "Mat setup",
      assistentePasso: "Step",
      assistenteDe: "of",
      assistenteVoltar: "Back",
      assistenteContinuar: "Continue",
      assistenteAjustar: "Adjust",
      assistenteDias: "Days",
      assistenteTempo: "Match time",
      assistenteDistribuicao: "Categories per day",
      assistenteOrdem: "Order of the day",
      assistenteAreas: "Mats",
      assistenteResumoDia: "day",
      assistenteResumoDias: "days",
      assistenteResumoPadrao: "CBJJ default",
      assistenteResumoAjustados: "adjusted",
      assistenteResumoOrdemPropria: "your order",
      assistenteResumoAuto: "automatic",
      assistenteResumoSemAreas: "to define",
      // collapse/expand sections on the mats screen
      secaoMostrar: "Show",
      secaoOcultar: "Hide",
      // search across the distributed schedule
      buscar: "Search",
      buscarFechar: "Close search",
      buscarPlaceholder: "Search athlete, category or mat…",
      buscarVazio: "Nothing found",
      buscarDigite: "Type to search by athlete, category or mat.",
      buscarResultado: "result",
      buscarResultados: "results",
      buscarMais: "more results — refine your search",
      buscarSemChave: "bracket not generated",
      imprimirPrograma: "Print schedule",
      programaTitulo: "Schedule",
      colHora: "Time",
      colArea: "Mat",
      colCategoria: "Category",
      colLutas: "Matches",
      geradoEm: "Generated on",
      // warning when the estimated matches don't fit the period
      naoCabePre: "The estimated matches (~",
      naoCabeMeio: " on the busiest mat) don't fit the available period (~",
      naoCabePos: " per mat).",
      naoCabeAreasPre: " Use at least ",
      naoCabeAreasPos: " mats, or extend the days/times.",
      naoCabeTempo: " Extend the daily times or add another day.",
      recomendacao: {
        titulo: "Recommended mats",
        ocultar: "hide",
        mostrar: "show",
        tatameIdeal: "ideal mat",
        tatamesIdeais: "ideal mats",
        unidade: "mat",
        unidadePlural: "mats",
        ocupacaoLabel: "of the window on the busiest mat",
        ocupacaoCom: "with",
        lutasPrevistas: "Estimated matches",
        tempoDeLuta: "Mat time",
        janelaPorTatame: "Window per mat",
        semInscricoes:
          "No confirmed registrations yet to estimate the matches — the recommendation shows up as soon as there are.",
        soMaisTempo:
          "A single division already runs past a full day. More mats won't help: extend the times or add another day.",
        nemComPre: "The matches don't fit the window even with ",
        nemComPos: ". Extend the times or add another day.",
        comecePre: "Start with ",
        comeceFimSing: ": it covers every match within the window.",
        comeceFimPlural: ": they cover every match within the window.",
        faltamPre: "With ",
        faltamMeio: " the matches don't fit the window. Use ",
        faltamPos: " to finish on time.",
        sobramPre: "You planned ",
        sobramMeio: ", and ",
        sobramFimSing: " would already do — the rest is slack.",
        sobramFimPlural: " would already do — the rest is slack.",
        bateFimSing: " covers every match within the window.",
        bateFimPlural: " cover every match within the window.",
      },
    },
    erros: {
      soRascunhoExcluir:
        "Only draft events can be deleted — this one is already published.",
      eventoComInscricoes:
        "This event already has registrations or payments recorded and can't be deleted.",
      nomeDataObrigatorios: "Event name and date are required.",
      inscricoesFecham: "Registration must close on or before the event date.",
      numAreasInvalido: "Enter a number of mats between 1 and 40.",
      diasObrigatorio: "Add at least one day with a time window for the event.",
      diaJanelaInvalida:
        "Each day needs a valid start and end, with the end after the start.",
      diaJanelaSobreposta:
        "Windows on the same day can't overlap — leave a break between them.",
      selecaoVazia:
        "The selection generates no divisions — pick at least one class, one sex and one belt.",
      categoriaComInscritos:
        "A division with entrants can't be deleted — move the athletes first.",
      blocoInvalido: "Invalid division block for setting a price group.",
      grupoInvalido: "Invalid price group.",
      soPublicadosEncerram:
        "Only published events can have registration closed.",
      publicarFalhou: "Couldn't publish the event",
      publicar: {
        nao_encontrado: "Event not found",
        ja_publicado: "Event already published",
        sem_categoria:
          "To publish, generate at least 1 division (use the category grid generator below).",
        sem_lote: "To publish, create at least 1 registration tier.",
      },
      loteCampos: "Fill in the tier's name, price and dates.",
      loteFimAntesInicio: "The tier's end must be after its start.",
      loteConflitoPre: "The dates overlap tier",
      loteConflitoPos: "Each tier needs a period separate from the others.",
      pacoteCampos: "Each price package needs a group and a value.",
      grupoInvalidoNome: "Invalid price group:",
      grupoRepetido: "Price group repeated in the tier:",
      chaveGerarFalhou: "Error generating the bracket",
      chave: {
        chave_min_inscricoes:
          "The division needs at least 2 confirmed registrations",
        chave_em_andamento:
          "A bracket in progress can't be regenerated — a result was already recorded",
      },
      nenhumaAguardandoChave:
        "No division awaiting a bracket (2+ confirmed and no bracket yet)",
      chavesGeradasPre: "Generated",
      chavesComFalhas: "failures —",
      nenhumaRascunhoPublicar: "No draft bracket to publish",
    },
  },
  regulamentoTitulos: {
    regrasInscricao: "Registration rules",
    pesagem: "Weigh-in",
    regrasLuta: "Match rules",
    temposLuta: "Match times",
    chaveamento: "Bracketing",
    woChamada: "Walkover and athlete calls",
    uniforme: "Uniform",
    arbitragem: "Refereeing",
    segurancaMedico: "Safety and medical care",
    conduta: "Conduct and discipline",
    premiacao: "Awards",
    absoluto: "Absolute",
    criancas: "Rules for children",
    estrutura: "Event structure",
    alteracaoCancelamento: "Change and cancellation policy",
    termosObrigatorios: "Mandatory terms",
  },
};
