    ...
    <div className="mx-auto max-w-4xl px-6 py-11">
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <h1 className="disp text-[56px]">{da.dashboard.meusEventos}</h1>
        </div>
        <Link
        ...
        >
          <SkewTexto>{da.dashboard.novoEvento}</SkewTexto>
        </Link>
      </div>

      {meusEventos.length === 0 ? (
        <div className="mt-14 border border-dashed border-white/16 p-12 text-center">
          <p className="disp text-[32px] text-white/30">
            {da.dashboard.nenhumEvento}
          </p>
          <p className="mt-2 text-[15px] font-medium text-muted-2">
            {da.dashboard.criePrimeiro}
          </p>
        </div>
      ) : (
